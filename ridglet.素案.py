...
b2_o = tf.Variable(b2.T,dtype=tf.float32)
...
y_o = tf.nn.softmax(tf.add(tf.matmul(y2_o,W3_o) ,b3_o))
...
train_step = tf.train.GradientDescentOptimizer(0.01).minimize(cross_entropy,var_list=[W,b,W2,W3,b2,b3])

cross_entropy_o = -tf.reduce_sum(y_*tf.log(y_o))
train_step_o = tf.train.GradientDescentOptimizer(0.01).minimize(cross_entropy_o,var_list=[W_o,b_o,W2_o,W3_o,b2_o,b3_o])
